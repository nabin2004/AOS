...
def test_settings_auto_export_ollama_base_url():
    with patch.dict(os.environ, {"EDUCLAW_MODEL": "ollama:aos-qwen3-8b-narrated-sft"}, clear=True):
        settings = Settings.from_env()
        assert settings.model == "ollama:aos-qwen3-8b-narrated-sft"
...
def test_resolve_educlaw_model_ollama():
    resolved = resolve_educlaw_model("ollama:aos-qwen3-8b-narrated-sft")
...
def test_build_agent_with_ollama_model():
    with patch.dict(os.environ, {"EDUCLAW_MODEL": "ollama:aos-qwen3-8b-narrated-sft", "OLLAMA_BASE_URL": "http://localhost:11434/v1"}):
        settings = Settings.from_env()

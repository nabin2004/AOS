#!/usr/bin/env python3
"""Pre-flight checks before a long Gemma 4 SFT run.
    ...
    *,
    ...
    if config.assistant_only_loss:
        ...
        if "Render failed: Axes not defined" in masked_text:
            ...
            return 1
        if "<|tool_call>" not in masked_text and "tool_call" not in masked_text:
            print("\nFAIL: tool_call tokens missing from assistant loss mask")
            return 1
        print(
            f"\nOK: assistant_masks ({mask_sum} / {len(messages)} message turns trained)"
        )

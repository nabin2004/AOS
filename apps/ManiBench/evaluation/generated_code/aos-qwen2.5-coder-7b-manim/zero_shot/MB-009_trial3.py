...
class FTCAnimation(Scene):
    def construct(self):
        ...
        for x in [0.5, 1, 1.5, 2]:
            ...
            # Update visuals
            self.play(MoveAlongPath(dot, Line(axes.c2p(0, 0), axes.c2p(x, 0))))
            self.play(Transform(area, new_area))
        ...
        # Final conclusion
        final_eq = MathTex(r"\int_0^x f'(t) \,dt = f(x) - f(0)").scale(1.5).to_edge(DOWN)

...
class ConvolutionAnimation(Scene):
    def construct(self):
        ...
        for i in range(len(signal_data)):
            ...
            # Update output chart
            output_bar_chart.update_values([conv_result if j == i else 0 for j in range(len(signal_data))])

            # Move kernel one step right
            self.play(kernel_bar_chart.animate.shift(RIGHT * 1.5))
            self.wait(0.5)

            # Clear signal slice highlight
            self.play(signal_slice.animate.set_fill(BLUE, opacity=1))

...
**aos** - FastAPI application generated with [Full-Stack AI Agent Template](https://github.com/vstorm-co/full-stack-ai-agent-template).
...
# RAG
uv run aos rag-collections
uv run aos rag-ingest /path/to/file.pdf --collection docs
uv run aos rag-search "query" --collection docs
uv run aos rag-sync-gdrive --collection docs
uv run aos rag-sync-s3 --collection docs

# Sync Sources
uv run aos cmd rag-sources
uv run aos cmd rag-source-add
uv run aos cmd rag-source-sync
```

...
```bash
uv run aos cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run aos cmd rag-search "your question" --collection docs        # semantic search
uv run aos cmd rag-collections                                     # list collections
uv run aos cmd rag-stats                                           # chunk/vector counts
uv run aos cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run aos cmd rag-sources                  # list
uv run aos cmd rag-source-add               # add (interactive)
uv run aos cmd rag-source-sync --all        # trigger a sync
```

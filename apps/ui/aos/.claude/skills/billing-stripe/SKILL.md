...
**Add/adjust a plan:** update `pricing.py` and the corresponding Stripe product/price; the `sync-stripe-plans` command (`aos cmd sync-stripe-plans`) mirrors Stripe → local `plan`/`price` tables.

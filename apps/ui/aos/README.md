...
└── commands/             # Click CLI commands (auto-discovered by `aos cmd …`)
...
The generated project ships a Click CLI exposed as `aos` (after `make install`):

```bash
aos server run --reload          # dev server
aos db upgrade                   # apply migrations
aos db migrate -m "message"      # create new migration
aos user create-admin            # interactive admin creation
aos rag-ingest <path> -c docs    # ingest local files
aos rag-search "query" -c docs   # semantic search
aos rag-collections              # list collections
aos celery worker                # start worker
aos celery beat                  # start scheduler
```

Run `make help` for a categorized list, or `aos --help` for full CLI docs.
...
aos rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from Google Drive (service-account auth)
aos rag-sync-gdrive --collection documents --folder-id <id>
# Pull from S3 / MinIO
aos rag-sync-s3 --collection documents --prefix docs/

# Semantic search
aos rag-search "your query" --collection documents
```

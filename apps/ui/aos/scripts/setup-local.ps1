...
if (-not $SkipStack) {
    ...
    Write-Host ""
    Write-Host "Celery for Animate = HOST (auto via refresh). Docker celery_worker cannot run uv/Manim." -ForegroundColor Yellow
    Write-Host "Start / refresh Animate worker:"
    Write-Host "     .\scripts\dev-refresh.ps1"
    Write-Host "     .\scripts\dev-refresh.ps1 -Rebuild   # Dockerfile / deps changed"
    Write-Host "     .\scripts\dev-refresh.ps1 -Logs      # host celery log + minio"
    Write-Host "     .\scripts\diagnose-animate.ps1       # hop-by-hop PASS/FAIL"
} else {
...
Write-Host "     .\scripts\dev-refresh.ps1              # up + HOST Celery"
Write-Host "     .\scripts\dev-refresh.ps1 -Rebuild     # rebuild + recreate app/flower"
Write-Host "     .\scripts\diagnose-animate.ps1         # E2E hop check"

...
class BrandedBackpropDeck(VoiceoverSlideScene):
    """Premium educational video with branded intro, opening music, mathematical animations,
    neural network graph, and word-level synchronized voiceover.
    """
    ...
    def construct(self):
        ...
        # 3. Slide 1: The Learning Paradigm (Forward Pass -> Loss -> Update)
        self.play_slide_1_learning_paradigm()

        # 4. Slide 2: Optimization Landscape (Mathematical Graph & Gradient Descent)
        self.play_slide_2_optimization_landscape()

        # 5. Slide 3: Credit Assignment (Neural Network & Chain Rule Animation)
        self.play_slide_3_credit_assignment()

        # 6. Slide 4: First-Principles Understanding (The Feynman Approach)
        self.play_slide_4_feynman_approach()

        # 7. Slide 5: Summary & Next Steps (Rich Visuals & Roadmap)
        self.play_slide_5_summary_and_next_steps()

    # -------------------------------------------------------------------------
    # Helper: Slide Chrome
    # -------------------------------------------------------------------------
    def create_slide_chrome(self, slide_num: str, title: str, category: str = "NEURAL NETWORKS") -> VGroup:
        """Create persistent header, category badge, divider, and footer."""
        cat_badge = Text(category, font_size=16, color=TEAL_C, weight="BOLD").to_corner(UP + LEFT, buff=0.45)
        badge_box = SurroundingRectangle(cat_badge, color=TEAL, buff=0.08, corner_radius=0.05, stroke_width=1.2)
        badge_group = VGroup(badge_box, cat_badge)

        header_title = Text(f"{slide_num}  |  {title}", font_size=26, color=WHITE, weight="BOLD")
        header_title.next_to(badge_group, RIGHT, buff=0.4)

        divider = Line(LEFT * 6.5, RIGHT * 6.5, color=DARK_GRAY, stroke_width=1.5).shift(UP * 2.7)
        
        footer_brand = Text("AOS Lecture Series  •  Backpropagation", font_size=14, color=GRAY).to_corner(DOWN + LEFT, buff=0.4)
        footer_page = Text(slide_num, font_size=14, color=GRAY).to_corner(DOWN + RIGHT, buff=0.4)

        return VGroup(badge_group, header_title, divider, footer_brand, footer_page)
    ...
    def play_branding_intro(self):
        ...
        # Step 1: Initial RUKUMINI Brand Screen
        ...
        self.play(Create(box), FadeIn(nabin, shift=UP * 0.2), run_time=0.8)
        self.wait(1.0)

        # Step 2: CLEAN EXIT of brand before title card (NO OVERLAP)
        self.play(FadeOut(VGroup(brand, box, nabin)), run_time=0.45)
        self.clear()

        # Step 3: Clean Title Card
        ...
        self.play(FadeIn(VGroup(lec_box, lec_badge), shift=DOWN * 0.2), FadeIn(topic, shift=UP * 0.2), FadeIn(subtitle, shift=UP * 0.2), run_time=1.0)
        self.wait(1.2)

        # Step 4: CLEAN EXIT of title card before Slide 1
        self.play(FadeOut(VGroup(lec_box, lec_badge, topic, subtitle)), run_time=0.5)
        self.clear()

    # -------------------------------------------------------------------------
    # 2. Slide 1: The Learning Paradigm
    # -------------------------------------------------------------------------
    def play_slide_1_learning_paradigm(self):
        chrome = self.create_slide_chrome("01", "The Learning Paradigm", "FOUNDATIONS")
        ...
        with self.voiceover(text=narration_text):
            ...
            self.play(FadeIn(card1, shift=UP * 0.3), run_time=0.7)

            self.wait_until_bookmark("p2")
            self.play(FadeIn(card2, shift=UP * 0.3), run_time=0.7)

            self.wait_until_bookmark("p3")
            self.play(FadeIn(card3, shift=UP * 0.3), run_time=0.7)
    ...
    # -------------------------------------------------------------------------
    # 3. Slide 2: Optimization Landscape (Mathematical Graph Animation)
    # -------------------------------------------------------------------------
    def play_slide_2_optimization_landscape(self):
        chrome = self.create_slide_chrome("02", "Optimization Landscape - Gradient Descent", "CALCULUS")
        self.play(FadeIn(chrome), run_time=0.5)

        # Coordinate Axes for Loss Parabola
        axes = Axes(
            x_range=[-1.0, 3.5, 1.0],
            y_range=[0, 4.5, 1.0],
            x_length=6.0,
        ...
        # Tangent Line at w0 (slope = 2*(2.6 - 1.2) = 2.8)
        tangent = Line(
        ...
        with self.voiceover(text=narration_text):
            self.play(Create(axes_group), Create(parabola), Write(curve_label), run_time=1.2)

            self.wait_until_bookmark("p1")
            self.play(FadeIn(dot, scale=1.3), Write(dot_label), FadeIn(info_group, shift=LEFT * 0.2), run_time=0.9)

            self.wait_until_bookmark("p2")
            self.play(Create(tangent), Write(grad_label), run_time=0.8)

            self.wait_until_bookmark("p3")
            # Step 1 towards minimum
            w1 = 1.8
    ...
    # -------------------------------------------------------------------------
    # 4. Slide 3: Credit Assignment (Neural Network & Chain Rule Animation)
    # -------------------------------------------------------------------------
    def play_slide_3_credit_assignment(self):
        chrome = self.create_slide_chrome("03", "Credit Assignment - The Chain Rule", "ARCHITECTURE")
        self.play(FadeIn(chrome), run_time=0.5)
        ...
        # Bottom Formula Card: Chain Rule
        ...
        back_arrow1 = CurvedArrow(loss_box.get_top() + LEFT * 0.1, y_hat.get_top() + RIGHT * 0.1, color=RED, angle=-0.5).shift(UP * 0.1)
        back_arrow2 = CurvedArrow(y_hat.get_top() + LEFT * 0.1, h1.get_top() + RIGHT * 0.1, color=ORANGE, angle=-0.5).shift(UP * 0.1)
        back_arrow3 = CurvedArrow(h1.get_top() + LEFT * 0.1, x1.get_top() + RIGHT * 0.1, color=YELLOW, angle=-0.5).shift(UP * 0.1)
        grad_tag1 = MathTex(r"\frac{\partial \mathcal{L}}{\partial \hat{y}}", font_size=18, color=RED).next_to(back_arrow1, UP, buff=0.05)
        grad_tag2 = MathTex(r"\frac{\partial \hat{y}}{\partial h_1}", font_size=18, color=ORANGE).next_to(back_arrow2, UP, buff=0.05)
        grad_tag3 = MathTex(r"\frac{\partial h_1}{\partial w_1}", font_size=18, color=YELLOW).next_to(back_arrow3, UP, buff=0.05)
        ...
        with self.voiceover(text=narration_text):
            ...
            self.wait_until_bookmark("p1")
            # Forward signal pulses
            pulse_x1 = Dot(x1.get_center(), radius=0.1, color=BLUE_C)
            pulse_h1 = Dot(h1.get_center(), radius=0.1, color=TEAL_C)
            self.play(
                pulse_x1.animate.move_to(h1.get_center()),
                run_time=0.6,
            )
            self.remove(pulse_x1)
            self.play(
                pulse_h1.animate.move_to(y_hat.get_center()),
                run_time=0.6,
            )
            self.remove(pulse_h1)
            self.play(FadeIn(chain_group, shift=UP * 0.2), run_time=0.8)

            self.wait_until_bookmark("p2")
            # Animate Backward Flow
            self.play(Create(back_arrow1), Write(grad_tag1), run_time=0.7)
            self.play(Create(back_arrow2), Write(grad_tag2), run_time=0.7)
            self.play(Create(back_arrow3), Write(grad_tag3), run_time=0.7)

            self.wait_until_bookmark("p3")
            # Credit assignment glow
            self.play(
                Indicate(x1, color=GREEN),
                Indicate(h1, color=GREEN),
                Indicate(y_hat, color=GREEN),
                run_time=1.0,
            )
    ...
    # -------------------------------------------------------------------------
    # 5. Slide 4: First-Principles Understanding (The Feynman Approach)
    # -------------------------------------------------------------------------
    def play_slide_4_feynman_approach(self):
        chrome = self.create_slide_chrome("04", "The Feynman Approach", "PEDAGOGY")
        self.play(FadeIn(chrome), run_time=0.5)
        ...
        quote_group = VGroup(q_box, quote_mark, quote_text, quote_author)
        ...
        with self.voiceover(text=narration_text):
            self.play(FadeIn(quote_group, shift=DOWN * 0.2), run_time=1.0)

            self.wait_until_bookmark("p1")
            self.play(FadeIn(card1, shift=UP * 0.2), run_time=0.7)

            self.wait_until_bookmark("p2")
            self.play(FadeIn(card2, shift=UP * 0.2), run_time=0.7)

            self.wait_until_bookmark("p3")
            self.play(FadeIn(card3, shift=UP * 0.2), run_time=0.7)
    ...
    # -------------------------------------------------------------------------
    # 6. Slide 5: Summary & Next Steps (Eliminating Dead Visual Time)
    # -------------------------------------------------------------------------
    def play_slide_5_summary_and_next_steps(self):
        chrome = self.create_slide_chrome("05", "Summary & Next Steps", "CONCLUSION")
        self.play(FadeIn(chrome), run_time=0.5)
        ...
        # Bottom Announcement Banner for Next Session
        ...
        next_code = MathTex(r"\mathbf{dW} = \frac{1}{m} \mathbf{dZ} \cdot \mathbf{A}^{[l-1]T}, \quad \mathbf{db} = \frac{1}{m} \sum \mathbf{dZ}", font_size=22, color=YELLOW).next_to(next_title, DOWN, buff=0.15)
        banner_group = VGroup(banner_box, next_badge, next_title, next_code)
        ...
        with self.voiceover(text=narration_text):
            self.play(FadeIn(top_cards, shift=DOWN * 0.2), run_time=0.9)

            self.wait_until_bookmark("p1")
            self.play(FadeIn(banner_box), Write(next_badge), Write(next_title), run_time=0.8)

            self.wait_until_bookmark("p2")
            self.play(Write(next_code), Indicate(banner_box, color=TEAL_C), run_time=0.9)
